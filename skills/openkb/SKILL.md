...
name: openkb
description: |
  Use when the user asks about content in their okforge knowledge base
  — research topics, concepts compiled from their documents,
  cross-document synthesis — or mentions `openkb`, an `.okforge/` (or
  legacy `.openkb/`) directory, or a `wiki/` tree generated by okforge. The user may
...
- **Concept pages** at `wiki/concepts/*.md` — cross-document synthesis
  on specific topics. This is where okforge's value compounds: a
  concept with multiple sources represents knowledge merged across
  documents the user has ingested. On some KBs concepts nest under
  `wiki/concepts/<topic>/` instead of sitting flat (see "topic tree"
  in `references/wiki-schema.md`) — wikilinks still resolve either
  way, but a raw directory listing won't be flat.
...
looking for `.okforge/` (or a not-yet-migrated KB's legacy `.openkb/`
— both work identically), then falls back to the global default set
by `openkb use`, so this works even when the user's cwd is unrelated to
the KB.
...
concepts are cross-document synthesis** — the core value okforge adds.
...
- `openkb recompile [doc]` — re-runs compilation, rewrites wiki pages
- `openkb reindex` — LLM-cost topic-tree clustering, moves concept files
...
- `openkb init` / `openkb use` — mutate `.okforge/` or global config
- `openkb migrate` — moves a KB's legacy `.openkb/` to `.okforge/`
- Direct edits to any file under `<kb>/wiki/` or `<kb>/.okforge/`
  (this is the user's curated content; don't patch it directly)

`openkb okf-lint` and `openkb describe` (no argument) are read-only
and safe to run when the user asks about wiki health or the project
description.
