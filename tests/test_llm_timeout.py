...
test_config.py) and read at the LiteLLM call sites in okforge.agent.compiler.
...
from okforge.agent.compiler import _llm_call, _llm_call_async
from okforge.config import set_timeout
...
def test_llm_call_forwards_configured_timeout():
    set_timeout(1200.0)
    with patch(
        "okforge.agent.compiler.litellm.completion", return_value=_fake_response()
    ) as completion:
...
def test_llm_call_async_forwards_configured_timeout():
    set_timeout(900.0)
    with patch(
        "okforge.agent.compiler.litellm.acompletion",
