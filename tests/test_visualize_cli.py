...
def test_visualize_writes_html_and_opens_by_default(tmp_path):
    kb = _kb(tmp_path)
    with patch("okforge.cli._find_kb_dir", return_value=kb), patch("webbrowser.open") as wb:
...
def test_visualize_empty_wiki(tmp_path):
    ...
    (tmp_path / ".okforge").mkdir()
    (tmp_path / ".okforge" / "config.yaml").write_text("model: gpt-4o-mini\n", encoding="utf-8")
    with patch("okforge.cli._find_kb_dir", return_value=tmp_path), patch("webbrowser.open") as wb:
        result = CliRunner().invoke(cli, ["visualize"])
