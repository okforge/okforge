...
This module is the input-acquisition layer for ``okforge add <URL>``: it
...
The caller (``okforge.cli.add``) then dispatches the saved path to the
normal local-file ingest pipeline (``add_single_file`` →
``convert_document`` → Markdown / PageIndex), so all the existing
...
def looks_like_url(s: str) -> bool:
    """Cheap check used by ``okforge add`` to branch into URL ingest."""
...
def _extract_html(url: str, raw_dir: Path) -> Path | None:
    ...
    if len(markdown) < _HTML_MIN_EXTRACT_CHARS:
        click.echo(
            ...
            f"resulting wiki entry and use `okforge remove` if it's empty.",
            err=True,
        )
...
def fetch_url_to_raw(url: str, kb_dir: Path) -> Path | None:
    ...
    existing PageIndex / Markdown routing by file extension and page
    ...
    click.echo(
        ...
        "Download the file manually and pass its path to `okforge add` instead.",
        err=True,
    )
    return None
