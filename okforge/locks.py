"""Cooperative filesystem locks and atomic writes for okforge.

The lock protocol is advisory and intended for local filesystem access by
okforge processes. It does not guarantee cross-host coordination on networked
...
def _drain_pending_journals(kb_state_dir: Path) -> None:
    ...
    those edits). ``kb_state_dir`` is the KB's state directory (see
    ``okforge.config.state_dir``) callers pass to ``kb_lock``; the journal
    lives at ``kb_state_dir/journal``, so the KB root is
    ``kb_state_dir.parent``.
    ...
    """
    from okforge.mutation import recover_pending_journals

    log = logging.getLogger(__name__)
    for message in recover_pending_journals(kb_state_dir.parent):
        log.warning(message)


@contextlib.contextmanager
def kb_lock(kb_state_dir: Path, *, exclusive: bool) -> Iterator[None]:
    """Hold a KB-level advisory lock."""
    lock_path = kb_state_dir / "ingest.lock"
    ...
    with local_context:
        with lock_path.open("a+", encoding="utf-8") as fh:
            ...
            try:
                if exclusive:
                    _drain_pending_journals(kb_state_dir)
                yield
            finally:
...
def kb_ingest_lock(kb_state_dir: Path):
    """Hold an exclusive KB mutation lock."""
    return kb_lock(kb_state_dir, exclusive=True)


def kb_read_lock(kb_state_dir: Path):
    """Hold a shared KB read lock."""
    return kb_lock(kb_state_dir, exclusive=False)
