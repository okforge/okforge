"""Structural lint checks for the okforge wiki.
...
from okforge.config import state_dir
from okforge.locks import atomic_write_text
from okforge.schema import EXCLUDED_WIKI_FILES, PAGE_CONTENT_DIRS

# Matches [[wikilink]] or [[subdir/link]]
_WIKILINK_RE = re.compile(r"\[\[([^\]]+)\]\]")

# Matches the destination of an inline markdown link ending in .md — the
# form the markdown link_style emits (see okforge/links.py). Image links
# never end in .md, so they don't match.
...
def _all_wiki_pages(wiki: Path) -> dict[str, Path]:
    ...
    for md in wiki.rglob("*.md"):
        ...
        pages[md.stem] = md
        # Topic-tree: a concept nested under concepts/<topic>/ keeps
        # answering to its flat concepts/<stem> address (the form links
        # were written with before the move).
        if rel.parts[0] == "concepts" and len(rel.parts) > 2:
            pages.setdefault(f"concepts/{md.stem}", md)
    return pages
...
def _extract_md_link_targets(text: str, own_dir: str) -> list[str]:
    """Return wiki targets (``concepts/x`` form) for relative markdown links.

    ``own_dir`` is the containing page's directory relative to the wiki root
    ("" for index.md). Absolute URLs and paths escaping the wiki are ignored.
    """
    import posixpath
    ...
    for raw in _MDLINK_RE.findall(text):
        ...
        if resolved.startswith(".."):
            continue
        out.append(resolved[: -len(".md")])
    return out


def _extract_page_links(text: str, own_dir: str) -> list[str]:
    """All link targets in a page — wikilinks plus relative markdown links."""
    return _extract_wikilinks(text) + _extract_md_link_targets(text, own_dir)
...
def list_existing_wiki_targets(wiki_dir: Path) -> set[str]:
    ...
    LLM-generated content to the wiki (e.g. ``okforge query --save``).
    ...
    *,
    ...
    Args:
        ...
        restrict_to: When provided, only rewrite these files (must live
            ...
            Used by ``okforge remove`` (issue #58 / Bug 2) to clean only
...
def find_broken_links(wiki: Path) -> list[str]:
    ...
    for md in wiki.rglob("*.md"):
        ...
        for target in _extract_page_links(text, own_dir):
...
def find_orphans(wiki: Path) -> list[str]:
    ...
    for md in all_mds:
        ...
        text = _read_md(md)
        own_dir = "/".join(md.relative_to(wiki).parts[:-1])
        outgoing[rel] = set(_extract_page_links(text, own_dir))
...
def find_missing_entries(
    ...
    *,
    ...
    the hash registry (``state_dir(kb_dir)/hashes.json``): registered files
    ...
    if kb_dir is not None:
        registry_file = state_dir(kb_dir) / "hashes.json"
        if registry_file.exists():
            # Deferred import: converter pulls in pymupdf,
...
def check_index_sync(wiki: Path) -> list[str]:
    ...
    index_links = set(_extract_page_links(index_text, ""))
    pages = _all_wiki_pages(wiki)
...
def _load_wiki_pages(wiki: Path) -> dict[Path, str]:
    ...
    :data:`PAGE_CONTENT_DIRS <okforge.schema.PAGE_CONTENT_DIRS>` paths.
    """
    pages: dict[Path, str] = {}
...
def find_invalid_frontmatter(
    ...
    into invalid YAML that okforge itself reads with string slicing but
...
def find_missing_okf_fields(
    ...
    ``type``; ``description`` is okforge's required one-liner. Only summaries/,
