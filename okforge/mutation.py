...
from okforge.locks import _fsync_directory, _target_mode, atomic_write_json
...
class MutationSnapshot:
    ...
    def track_new(self, paths: list[Path]) -> None:
        ...
        PageIndex ``{doc_id}`` blob under ``.okforge/files`` is named by indexing,
...
def _restore_hardlinked_dir(backup: Path, target: Path) -> None:
    ...
    whole tree on rollback — the cost that bit ``.okforge/files`` (the blob
...
def snapshot_paths(
    ...
    *,
    ...
    backup_dir = state_dir(kb_dir) / "staging" / f"rollback-{journal_id}"
    backup_dir.mkdir(parents=True, exist_ok=False)
    snapshot = MutationSnapshot(
        kb_dir=kb_dir,
        backup_dir=backup_dir,
        journal_path=state_dir(kb_dir) / "journal" / f"{journal_id}.json",
        operation=operation,
        details=details or {},
    )
    try:
...
def recover_pending_journals(kb_dir: Path) -> list[str]:
    ...
    journal_dir = state_dir(kb_dir) / "journal"
