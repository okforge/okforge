...
After every successful skill generation (CLI ``okforge skill new`` or
...
NOT auto-regenerated; re-run ``okforge skill new`` or
...
from okforge.skill import skills_root
...
def _build_manifest(kb_dir: Path) -> dict[str, Any]:
    ...
    # Naming convention is locked to `okforge@vectify` so users get one
    # canonical install command regardless of which KB they're consuming;
    # different KBs are distinguished by <owner>/<repo> URL.
    metadata_desc = f"Skills compiled from the {kb_dir.name} knowledge base via okforge."
    plugin_desc = "Knowledge skills compiled from this okforge-managed knowledge base."
