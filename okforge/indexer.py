...
def _normalize_page_content(raw_pages: Any) -> list[dict[str, Any]]:
    """Normalize PageIndex/local PDF page content into okforge's JSON shape."""
...
def _convert_pdf_to_pages(pdf_path: Path, doc_name: str, images_dir: Path) -> list[dict[str, Any]]:
    from okforge.images import convert_pdf_to_pages
...
def index_long_document(pdf_path: Path, kb_dir: Path, doc_name: str | None = None) -> IndexResult:
    ...
    # Local-only: PageIndex Cloud support was stripped from this fork, so no
    # API key is ever passed — the TOC/tree generation runs entirely locally.
    client = PageIndexClient(
        api_key=None,
        model=model,
        storage_path=str(kb_state_dir),
