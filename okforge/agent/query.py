...
from okforge.config import get_extra_headers, get_timeout_extra_args
from okforge.schema import get_agents_md
...
def build_query_agent(wiki_root: str, model: str, language: str = "en") -> Agent:
    ...
    tree_on = bool(
        load_config(state_dir(Path(wiki_root).parent) / "config.yaml").get("topic_tree", False)
    )
...
def build_chat_agent(
    ...
    This is the variant used by the interactive ``okforge chat`` REPL so users
    ...
    Skill discovery: ``okforge/agent/skills.scan_local_skills`` looks in
    ``<kb>/skills/``, ``~/.config/okforge/skills/``, ``~/.claude/skills/`` for
    ...
    from okforge.agent.skills import scan_local_skills
...
async def run_query(
    ...
    *,
    ...
    from okforge.agent.chat import (
        _build_style,
        _fmt,
        _format_tool_line,
        _make_markdown,
