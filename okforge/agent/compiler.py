"""Wiki compilation pipeline for okforge.
...
from okforge.schema import INDEX_SEED, get_agents_md
...
_SYSTEM_TEMPLATE = """\
You are okforge's wiki compilation agent for a personal knowledge base.
...
# Used instead of _SUMMARY_USER when a sibling wiki/sources/<doc_name>.json
# page array exists (dropped there by the converter when a .pages.json file
# sat next to the raw .md) — the "[Page N]" markers below are inserted by
# compile_short_doc, one per page array entry, so the LLM can cite real page
# numbers instead of writing a citation-free summary the way a flat,
# unmarked document forces it to.
_SUMMARY_USER_PAGED = """\
New document: {doc_name}

Full text, split into its original pages. Each page's text is preceded by \
a "[Page N]" marker showing that page's number in the source document:
{content}

Write a summary page for this document in Markdown. When you state a \
specific fact, finding, or quote, cite the page it came from in the form \
"(p. N)" immediately after it.

Return a JSON object with two keys:
- "description": A single sentence (under 100 chars) describing the document's main contribution
- "content": The full summary in Markdown. Include key concepts, findings, ideas, \
page citations as instructed above, and [[wikilinks]] to concepts that could \
become cross-document concept pages
...
# ``.okforge/config.yaml`` override the default enum everywhere at once. The
...
def _accepts_cache_control(model: str) -> bool:
    ...
    # caller patches ``okforge.agent.compiler.litellm`` to stub out completion.
...
def _emit_links(content: str, own_dir: str, wiki_dir: Path) -> str:
    """Apply the KB's configured link style to a generated page body.

    Default (``link_style: markdown``) converts ``[[wikilinks]]`` to relative
    markdown links per OKF; ``link_style: wikilinks`` leaves them as-is.
    """
    from okforge import links
    from okforge.config import load_config

    config = load_config(state_dir(wiki_dir.parent) / "config.yaml")
    if links.link_style(config) == "markdown":
        return links.wikilinks_to_markdown(content, own_dir, wiki_dir)
    return content
...
def _write_summary(
    ...
    summary = _emit_links(summary, "summaries", wiki_dir)
...
def _write_concept(
    ...
    try:
        own_dir = str(base_dir.resolve().relative_to(wiki_dir.resolve()))
    except ValueError:
        own_dir = "concepts"
    content = _emit_links(content, own_dir, wiki_dir)
...
def remove_doc_from_concept_pages(
    ...
    *,
    ...
    a newer version that will repopulate the source on the next ``okforge
...
async def compile_short_doc(
    ...
    # A sibling wiki/sources/<doc_name>.json page array (dropped there by
    # convert_document when a .pages.json sat next to the raw .md) means real
    # per-page text is available — use it instead of the flat file so the
    # summary prompt can ask for page citations.
    pages_json_path = wiki_dir / "sources" / f"{doc_name}.json"
    page_array = None
    if pages_json_path.exists():
        try:
            page_array = json.loads(pages_json_path.read_text(encoding="utf-8"))
        except (json.JSONDecodeError, OSError) as exc:
            logger.warning("Ignoring unreadable %s: %s", pages_json_path, exc)
            page_array = None
    ...
    if page_array:
        ...
        summary_user_template = _SUMMARY_USER_PAGED
    else:
        # Strip the OKF frontmatter the converter writes on sources/*.md —
        # it's bundle metadata, not document text for the LLM.
        content = frontmatter.body(source_path.read_text(encoding="utf-8"))
        summary_user_template = _SUMMARY_USER
