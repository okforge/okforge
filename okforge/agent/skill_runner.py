...
This decouples generators from hard-coded prompts. ``okforge deck new`` /
``okforge skill new`` / any future ``okforge <type> new`` command becomes a
...
* ``deck_grammar`` — passed to :func:`okforge.deck.validator.validate_deck`
...
from okforge.agent.query import build_query_agent
from okforge.agent.skills import _parse_frontmatter, scan_local_skills
from okforge.agent.tools import read_kb_file, write_kb_file
...
class SkillRunResult:
    ...
    validation: Optional[Any] = None  # okforge.deck.validator.ValidationResult
    ...
    *,
    ...
    Args:
        ...
        extra_skill_roots: Additional directories to scan beyond the
            built-in ``<kb>/skills``, ``~/.config/okforge/skills``,
            ``~/.claude/skills``.
    ...
    if match is None:
        ...
        raise SkillNotFoundError(
            ...
            f"Drop a SKILL.md into ~/.config/okforge/skills/<name>/ or "
    ...
    if od_meta.get("mode") == "deck" and output_path is not None:
        ...
        from okforge.deck.validator import DeckGrammar, validate_deck
