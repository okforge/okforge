...
Builds on the single-shot Q&A agent in ``okforge.agent.query`` and keeps
...
from okforge.agent.query import MAX_TURNS, build_chat_agent
...
def _make_prompt_session(
    ...
    history_path = state_dir(kb_dir) / "chat_history"
...
async def _run_add(arg: str, kb_dir: Path, style: Style) -> None:
    ...
    from okforge.cli import SUPPORTED_EXTENSIONS, add_single_file
...
async def _handle_slash_skill(arg: str, kb_dir: Path, style: Style) -> None:
    ...
    if result is not None and (result.errors or result.warnings):
        ...
        _fmt(
            ...
            (
                "class:slash.help",
                f"Run `okforge skill validate {name}` to re-check, or "
...
async def _handle_slash_deck(arg: str, kb_dir: Path, style: Style) -> None:
    ...
    from okforge.deck import deck_dir
...
async def _handle_slash(
    ...
    if head == "/status":
        from okforge.cli import print_status
        from okforge.locks import kb_read_lock
    ...
    if head == "/list":
        from okforge.cli import print_list
        from okforge.locks import kb_read_lock
    ...
    if head == "/lint":
        from okforge.cli import run_lint
