...
def okf_meta_lines(title: str) -> list[str]:
    """OKF-recommended metadata lines: ``title`` + generation ``timestamp``."""
    return [kv_line("title", title), kv_line("timestamp", okf_timestamp())]


def refresh_okf_meta(fm_block: str, title: str) -> str:
    """Set/refresh ``title`` + ``timestamp`` in an existing frontmatter block."""
    fm_block = set_line(fm_block, "title", title)
    return set_line(fm_block, "timestamp", okf_timestamp())
